    ...
    if p := st.chat_input("Consulta algo..."):
        ...
        if client:
            try:
                ...
                # ✅ FIX: limpieza segura sin saltos de línea literales en strings
                raw_sql = res_sql.choices[0].message.content.strip()
                sql_final = re.sub(r"```(?:sql)?", "", raw_sql).strip()
...
if __name__ == "__main__":
    main()    posibles_deptos = [c for c in df.columns if any(x in c for x in ["dep", "area", "div"])]
    posibles_status = [c for c in df.columns if any(x in c for x in ["est", "situ", "motivo", "asist"])]
